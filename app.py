...
import babel
from flask import Flask, render_template, request, Response, flash, redirect, sessions, url_for
...
def search_venues():
  ...
  upcoming_shows = Show.query.join(Venue, Artist).filter(Show.venue_id==Venue.id, Show.start_time > datetime.now()).all()
...
def show_venue(venue_id):
  ...
  shows_past = Show.query.join(Venue, Artist).filter(Show.venue_id==Venue.id, Show.start_time < datetime.now()).all()

 
  future_shows = Show.query.join(Venue, Artist).filter(Show.venue_id==Venue.id, Show.start_time > datetime.now()).all()
...
def create_venue_submission():
    ...
    try:
        ...
        venue = Venue(name=name, state=state, city=city, address=address,
...
def search_artists():
  ...
  upcoming_shows = Show.query.join(Venue, Artist).filter(Show.artist_id==Artist.id, Show.start_time > datetime.now()).all()
